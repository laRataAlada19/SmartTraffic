...
def process_video(video_file, model, total_class_counter, time_of_start, location_id):
    ...
    while cap.isOpened():
        ...
        frame, track_history,speed_violations = process_frame(frame, model, detected_vehicles, class_counter, track_history,speed_limited_of_street,speed_violations)
    ...
    if not db.exists_result(ultimo_tempo_guardado, id):
        ...
        print("Veiculos em excesso:", len(speed_violations))
        print("Velocidade média de todos os veículos:", avg_speed)
        db.save_results_to_bd(class_counter, total_class_counter, ultimo_tempo_guardado, id,len(speed_violations),avg_speed)
    else:
