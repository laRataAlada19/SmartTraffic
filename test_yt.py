    ...
    for result in track_results:
        ...
        if zoneA_Line[0] < cx < zoneA_Line[2] and zoneA_Line[1] - 20 < cy < zoneA_Line[1] + 20:
            if zoneAcounter.count(id) == 0:
                ...
                if class_detect == 'car':
                    total_cars += 1
                elif class_detect == 'truck':
                    total_trucks += 1
                elif class_detect == 'bus':
                    total_buses += 1
...
#show results
print(f"Total Detection: {len(zoneAcounter) + len(zoneBcounter) + len(zoneCcounter)}")
print(f"LANE A Vehicles = {len(zoneAcounter)}")
print(f"LANE B Vehicles = {len(zoneBcounter)}")
print(f"LANE C Vehicles = {len(zoneCcounter)}")
print(f"Total Cars = {total_cars}")
print(f"Total Trucks = {total_trucks}")
print(f"Total Buses = {total_buses}")
print(f"Total Detections Wanted: {total_buses + total_cars + total_trucks}")
